...
nr = function(f, x0, e, d){
    ...
    x1 = nr_inc(f, x0, d)
    while(abs(x1-x0)>e){
        # While we haven't converged, iterate the NR algorithm (nr_inc)
        x0 = x1
        x1 = nr_inc(f, x0, d)
        if(r){
            print(x1)
...
for(xi in xis){
    xmax = c(xmax, nr(f_find_0, xi, eps, delt))
}
```
...
g = qplot(df$xss, df$yss) +
    geom_line() +
    ggtitle(paste("First derivative of log likelihood collocated at", N, "points"))
g
```

Note that N-R finds the obvious roots of the first derivative of the log
likelihood function, but it diverges on the tails.
