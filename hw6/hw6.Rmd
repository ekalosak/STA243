...
if(which_penalty=="AIC"){
    ...
    which_organism_is_best = sort(
                            which(as.numeric(pop$AIC) == best_score),
                            decreasing=T
                        )[1]
}else if(which_penalty=="MDL"){
    best_score = min(as.numeric(pop$MDL))
    which_organism_is_best = sort(
                            which(as.numeric(pop$MDL) == best_score),
                            decreasing=T
                        )[1]
}
best_chromosome = pop$Chromosome[which_organism_is_best]
plt_best1 = plot_chromosome(best_chromosome, genetic_df, color="coral") +
    ggtitle(
        paste(
            "Chromosome ", which_organism_is_best, " in generation ",
            pop$Generation[which_organism_is_best],
...
## Simulate
pop = genetic_piecewise_regression(y2s,
                                   G, K,
                                   Pcross, Pmutate, Pimmigrate, Psex,
                                   which_penalty)

## Plot results
genetic_df = data.frame(y=y2s, x=1:n)
...
best_chromosome = pop$Chromosome[which_organism_is_best]
plt_best2 = plot_chromosome(best_chromosome, genetic_df, color="coral") +
    ggtitle(
        paste(
            "Chromosome ", which_organism_is_best, " in generation ",
            pop$Generation[which_organism_is_best],
