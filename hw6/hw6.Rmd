...
## Simulate
pop1 = genetic_piecewise_regression(y1s, G, K,
...
plt_best1 = plot_chromosome(best_chromosome1, plot_df1, color="olivedrab") +
    ggtitle(
        paste(
...
## Simulate
pop2 = genetic_piecewise_regression(y2s, G, K,
                                   Pcross, Pmutate, Pimmigrate, Psex,
                                   which_penalty2)
...
bootstrap_residuals = function(B, best_chromosome, ys, which_penalty){

    ## Recover residuals
    yhats = regress_chromosome(best_chromosome, ys)
    res = ys - yhats
    ...
    for(b in 1:B){
        ...
        boot_pop = genetic_piecewise_regression(boot_ys,
        ...
        boot_yhats = regress_chromosome(boot_best_chrom, boot_ys)
...
point_ests1 = bootstrap_residuals(B, best_chromosome1, y1s, which_penalty1)
y1hats = regress_chromosome(best_chromosome1, y1s)
...
plt_boot_resid1 = ggplot(data=df_95c_resid1) +
    ...
    geom_line(aes(x=x, y=y), color="olivedrab", size=1.1) +
...
## Recover the bootstrapped pointwise quantiles
point_ests2 = bootstrap_residuals(B, best_chromosome2, y2s, which_penalty2)
y2hats = regress_chromosome(best_chromosome2, y2s)
