...
5. $tr(H_\lambda H_\lambda^\top) -2tr(H_\lambda) + n =
\sum_{i=1}^n (h_{\lambda,ii} - 1)^2$

6. $E(||(I-H_\lambda)e||^2) =
\sigma^2(n + tr(H_\lambda H_\lambda^\top) -2tr(H_\lambda)$ because
$Cov(e) = I\sigma^2$ and
$tr(H_\lambda H_\lambda^\top) - 2tr(H_\lambda) + n =
\sum_{i=1}^n h_{ii}^2 - 2h_{ii} + 1 = \sum_{i=1}^n (h_{ii}-1)^2 =
tr(I (H_\lambda-I)(H_\lambda-I)^\top I^\top)$.

completing the proof.

```{r}
er = function(X=X, Y=Y, k=k, l){
    # k is num knots
    # l is lambda, penalty
    # X is design matrix
    # Y is response
